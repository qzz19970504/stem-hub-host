...
10 秒开 / 50 秒关只是时间降额措施。v3.2 另由 MCU_C、LM51770_C、MP4317_C、DRV8874_C、CHARGE_MOS_C 五路器件 NTC 执行软件保护：任一路严格高于 60.0°C、无效或读取失败即安全停机；只有五路全部有效且不高于 55.0°C 才解除锁存。BATT_NTC 仅显示。软件保护仍不能替代硬件限流、功率器件选型和散热设计。
